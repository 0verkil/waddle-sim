...
class MechanismSimulator : FunSpec( {
    ...
    test("Mechanism Demo") {
        ...
        SimulatedHardware.addSimulationObject(LinearExtension("Slides", -0.15, 1.0, 11.0, 40.0, 45.0, 25.0, leftSlide, rightSlide))

        Simulation.addSimulation(MechanismDemo(), 120.0)

        Simulation.runNextSimulation { Logger.recordOutput("Simulation/Pose", Pose2d()); Logger.recordOutput("Simulation/Left", leftSlide.state.get(2)) }
